...
def _():
    ...
    root = str((Path(__file__).resolve().parents[1] / "lambda_distributions/proofs/nonuniform_lambda_distributions/compact_u1_heat/notebook.py").parents[3])
    if root not in sys.path:
        sys.path.insert(0, root)
    from lambda_distributions.proofs.nonuniform_lambda_distributions.compact_u1_heat.verification import run_suite
    return mo, run_suite
