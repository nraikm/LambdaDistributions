...
def _():
    ...
    import sys
    repo_root = str((Path(__file__).resolve().parents[1] / "proved_matrix_groups/packages/matrix_group_formula_verification/representation_variants/notebook.py").parents[3])
    if repo_root not in sys.path:
        sys.path.insert(0, repo_root)
    from for_this_guy.matrix_group_formula_verification.representation_variants.verification import run_suite
    return mo, run_suite
