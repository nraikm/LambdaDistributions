...
def _():
    ...
    import sys
    import marimo as mo
    repo = str((Path(__file__).resolve().parents[1] / "lambda_distributions/proofs/schur_functor_classical_groups/orthogonal/notebook.py").parents[4])
    if repo not in sys.path:
        sys.path.insert(0, repo)
    from lambda_distributions.proofs.schur_functor_classical_groups.orthogonal.verification import run_suite
    return mo, run_suite
